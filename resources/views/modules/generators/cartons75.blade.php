...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        ...
        <div class="flex items-center justify-center border">
            <div class="w-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-2 flex items-center justify-center border p-2 md:p-3 overflow-y-scroll">
                <!-- { { dd($cartons) } } -->
                @foreach($cartons->carton as $number => $carton)
                        ...
                        @foreach($carton as $letter => $series)
                            ...
                            <div class="rounded-lg flex flex-col justify-between mt-8">
                                ...
                                @foreach($series as $serie)
                                        <div class="{{ (($serie == 0) ? 'bg-gray-800' : 'bg-[#C18C1A]') }} rounded-sm md:rounded-lg flex items-center justify-center font-bold md:font-normal text-white text-[10px] md:text-[14px] transition-all duration-500 m-1 p-1 text-gray-800">
                                            <p class="{{ (($serie == 0) ? 'invisible' : '') }}">{{ $serie }}</p>
                                        </div>
                                @endforeach
                            </div>                                
                            @php $cols++ @endphp
