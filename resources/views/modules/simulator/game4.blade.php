<x-guest-layout>
    ...
    <!-- class="w-full rounded-lg overflow-hidden flex flex-col relative" -->
        ...
        <!-- <section class="w-full flex">
            ...
            <div class="w-full relative p-2 pb-12 flex flex-row flex-nowrap items-center justify-center">
                <div class="bg-transparent focus:bg-gray-200 focus:bg-opacity-50 hover:bg-gray-200 hover:bg-opacity-50 text-black w-full flex justify-around items-center absolute bottom-0 left-0 z-10">
                    ...
                    {{-- Buttons View Toggle Modes y Board --}}
                    ...
                    {{-- Buttons Left Rigth Scroll - Panel Cartons --}}
        ...
        <div class="grid grid-cols-6 grid-rows-5 gap-1 rounded-3xl border border-fuchsia-400/30 bg-white/[0.07] p-5 shadow-2xl shadow-fuchsia-950/40 ring-1 ring-fuchsia-500/10 backdrop-blur">
            ...
            <div class="row-span-2 row-start-2">
                <h2 class="mb-5 text-sm font-bold uppercase tracking-wide text-indigo-100 text-center">
                    {{ __('Bolillero') }}
                </h2>
            </div>

            {{-- Cartones --}}
            <div class="col-span-4 row-start-2 rounded-3xl border border-fuchsia-400/30 bg-white/[0.07] p-2 shadow-2xl shadow-fuchsia-950/40 ring-1 ring-fuchsia-500/10 backdrop-blur">
                <div class="grid grid-cols-1 content-evenly overflow-x-auto" :class="'md:grid-cols-'+(Object.keys(cartons).length)">
                    <template x-for="(carton, number) in cartons">
                        <div class="slider-item flex flex-row items-center justify-center">
            ...
            <div class="col-start-6">
                ...
                <!-- {{-- Ultima Bola --}}
                <div x-show="sequence.length > 0" class="flex flex-col justify-center items-center gap-1">
                    <span class="text-xs text-indigo-200/70">{{ __('Última bola') }}</span>
                    <div class="flex h-11 w-11 items-center justify-center rounded-full bg-fuchsia-500 font-black shadow-lg shadow-fuchsia-900/40 text-sm"
                        x-text="sequence[sequence.length - 1].letter+sequence[sequence.length - 1].number">
                    </div>
                </div> -->
                ...
                <template x-if="setting.section.submodes.extended">
                    <article class="w-auto overflow-x-auto" x-show="submodes.length">
                        <!-- <span class="text-xs text-indigo-200/70">{{ __('Modos Ganadores') }}</span> -->
                        <h2 class="mb-5 text-sm font-bold uppercase tracking-wide text-indigo-100 text-center">
                            {{ __('Modos Ganadores') }}
                        </h2>
                        <div class="flex flex-col items-center justify-center font-thin pb-4 rounded-t-md p-4 text-center" >
                            <template x-for="submode in submodes">
                                <div class="flex flex-col items-center justify-center m-1">
                                    ...
                                    <div class="w-[85px] h-[85px] bg-gray-400 rounded-md p-1 flex items-center justify-center">
                                        <template x-for="col in submode.columns">
                                            <div class="flex flex-col items-center justify-center">
                                                <template x-for="row in submode.rows">
                                                    <div :class="(submode.coordinates.findIndex(coord => coord.x == (col-1) && coord.y == (row-1) ) > -1) ? 'bg-indigo-950' : 'bg-gray-100'" class="p-[5px] m-[3px]"></div>
                                                </template>
                                            </div>
                                        </template>
                                    </div>
            ...
            {{-- Board --}}
            <div x-show="setting.section.board.enabled" class="col-span-5 col-start-2 rounded-3xl border border-fuchsia-400/30 bg-white/[0.07] p-5 shadow-2xl shadow-fuchsia-950/40 ring-1 ring-fuchsia-500/10 backdrop-blur overflow-hidden relative">

                <!-- <button type="button" class="w-10 h-10 p-4 hover:bg-fuchsia-100/30 absolute top-0 right-0 flex flex row justify-center items-center">
                    {{-- Icon Extends --}}
                    {{ __('View') }}&nbsp;{{ __('Board') }}
                ...
                <template x-if="setting.section.board.enabled && setting.section.board.extended">
                    <div class="flex flex-col gap-1 font-bold">
                        ...
                        <template x-for="row in board">
                            <div class="flex flex-row flex-nowrap justify-around items-center transition-all gap-2">
                                <div class="flex items-center justify-center transition p-1.5 w-10 h-10 rounded" x-text="row.letter"></div>
                                <template x-for="range in row.ranges">
                                    <div class="flex flex-row items-center justify-center transition p-1.5 w-10 h-10 rounded border-0" :class="(range.active) ? 'bg-fuchsia-400/50' : 'bg-white/5 text-indigo-100/50'" x-text="range.number"></div>
                                </template>
                            </div>
                        </template>
                    </div>
